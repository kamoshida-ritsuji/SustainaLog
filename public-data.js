...
function filterPublished(articles) {
  return articles
    .filter(a =>
      a.status === ARTCICLE_STATUS.PUBLISHED &&
...
async function loadPublicArticles() {
  ...
  if (!isSheetsConfigured()) {
    ...
    _publicSource = 'fallback'; _publicError = 'SPREADSHEET_ID ��設定';
    return;
  }
  try {
    ...
    console.info('[PublicData]', _publicArticles.length, '件公斋記事読み侼み中');
  } catch (err) {
...
function getPublicArticleBySlug(slug) { return getPublicArticles().find(a => a.slug === slug) || null; }
...
async function initPublicPage(mountId, onLoad) {
  ...
  if (el) el.innerHTML = '<div style="text-align:center;padding:64px 0;"><div style="width:36px;height:36px;border:3sx solid var(--ink-100);border-top-color:var(--green-500);border-radius:50%;animation:spin .7s linear infinite;margin:0 auto 16px;"></div></div>';
...
function getArticleBySlug(slug) { return getPublicArticleBySlug(slug); }
...
function getCompanySummaries() {
  ...
  articles.forEach(a => {
    ...
    if (!map[key]) {
      ...
      map[key] = {
        company_name:key, ticker:a.ticker||(m&&m.ticker)||null,
        market:a.market||(m&&m.market)||null, industry:a.industry||(m&&m.industry)||'',
        icon:(m&&m.icon)||'🏢', official_url:(m&&m.official_url)||null,
        sustainability_url:(m&&m.sustainability_url)||null, ir_url:(m&&m.ir_url)||null,
        articles:[], scores:[], theme_count:{},
      };
    }
    map[key].articles.push(a);
    if (a.total_score) map[key].scores.push(Number(a.total_score)||0);
    const t = a.csr_type;
    if (t) map[key].theme_count[t] = (map[key].theme_count[t]||0)+1;
  });
  return Object.values(map).map(c => {
    ...
    return {company_name:c.company_name,ticker:c.ticker,market:c.market,industry:c.industry,icon:c.icon,official_url:c.official_url,sustainability_url:c.sustainability_url,ir_url:c.ir_url,article_count:c.articles.length,avg_score:avg,themes:themes,latest_date:latest,articles:sorted};
